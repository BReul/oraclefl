 ...
 **/
const formatArrayCamelCase = R.pipe(
    R.prop('rows'),
    R.map(R.filter(isNotNil)),
    R.map(camelCase)
);

const formatArray = R.pipe(
    R.prop('rows')
);

// formats an expected single result from the SQL response as a JSON object
const formatOneJsonCamelCase = R.pipe(formatArrayCamelCase, R.head);
const formatOneJson = R.pipe(formatArray, R.head);
